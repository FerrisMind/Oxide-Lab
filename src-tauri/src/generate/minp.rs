...
impl MinPFilter {
    pub fn new(min_p: Option<f64>, temperature: f64) -> Self {
        Self { min_p: min_p.and_then(|v| if (0.0..=1.0).contains(&v) { Some(v as f32) } else { None }), temperature: temperature as f32, log_prints: 0 }
    }
    ...
    pub fn apply(&mut self, logits: &Tensor) -> Result<Tensor, String> {
        ...
        for &v in &vals {
            if v > max_val { second_val = max_val; max_val = v; }
