...
impl CandleLLM {
    ...
    pub fn generate(&self, prompt: &str, max_tokens: usize, temperature: f64) -> Result<String> {
        ...
        for _ in 0..max_tokens {
            ...
            // Получаем logits для последней позиции
            let last_logits = logits.narrow(1, (seq_len - 1) as i64, 1)?.squeeze(1)?;
            let last_logits = last_logits.squeeze(0)?;

            // Применение logits processor для сэмплинга
            ...
            // Проверяем на EOS после добавления
        ...
        // Декодирование
        let output_tokens = &generated_tokens[tokens.len()..]; // Только сгенерированные токены
