[![English](https://img.shields.io/badge/English-Inactive-lightgrey)](README.md) [![Русский](https://img.shields.io/badge/Русский-Active-success)](README-RU.md) [![Português](https://img.shields.io/badge/Português-Inactive-lightgrey)](README-PT-BR.md)
...
#### Отладка зависаний при загрузке модели

Чтобы понять, где именно «виснет» (фронтенд/бэкенд), включите расширенное логирование:

- **Frontend (DevTools Console):** выполните `localStorage.setItem('oxide.debugModelLoad','1')`, перезагрузите приложение и начните загрузку модели.
  - Появятся логи `"[load_progress]"` с интервалами между событиями и предупреждения `"[ui-stall]"`, если event loop блокируется.
- **Backend (Rust):** установите переменные окружения:
  - `OXIDE_DEBUG_MODEL_LOAD=1` — печать таймингов стадий и всех `load_progress` (с `load_id` и `+ms`).
  - `RUST_LOG=load=debug` (или `RUST_LOG=debug`) — чтобы увидеть debug-логи (по умолчанию уровень INFO).

Интерпретация:
- Если `ui-stall` срабатывает, а `load_progress` продолжает приходить — проблема на фронтенде (JS thread/рендер).
- Если `load_progress` перестаёт приходить на одной стадии — проблема в конкретном шаге бэкенда (смотрите последнюю стадию/тайминг).

#### Исправление «залипания» Windows (серый UI / системный спиннер / смена иконки)

Если при загрузке модели Windows показывает системный wait-cursor и «тускнеет» UI, это обычно означает, что тяжёлая нагрузка забирает CPU-время у WebView/рендера.
Oxide Lab автоматически снижает приоритет CPU-интенсивных потоков во время загрузки модели (Windows), чтобы интерфейс оставался отзывчивым.

Если на вашем ПК требуется ещё более плавный UI ценой скорости загрузки, можно включить процессный background mode:

- `OXIDE_WIN_BACKGROUND_MODE=1`
#### Ошибка сборки `core-foundation` / `objc2` на Windows

Если при сборке на Windows появляются ошибки про `core-foundation` или `objc2`, это связано с особенностью фичи `hf-hub` (включает `reqwest/macos-system-configuration`).
В проекте это исправлено патчем `hf-hub` в `patches/hf-hub-0.4.3` (см. `[patch.crates-io]` в `src-tauri/Cargo.toml`).
