...
2. Нажмите **«Выбрать файл модели»** и укажите путь к `.gguf`
3. Нажмите **«Выбрать токенизатор»** и укажите `tokenizer.json`
4. При необходимости настройте параметры инференса
5. Нажмите **«Загрузить»**
...
- Включайте «размышления» для сложных задач
...
- Можно менять значения по умолчанию. Экспериментируйте с температурой: 0.7–1.0 — креативность, 0.1–0.3 — точность
- Увеличивайте контекст для работы с длинными документами

### ⚡ **Оптимизация производительности:**
- В текущей версии работает только на CPU+RAM. Поддержка GPU (CUDA) запланирована
...
Самые маленькие модели Qwen3 (0.6B и 1.7B) работают с приемлемой скоростью и качеством даже на устройствах с 2‑ядерным CPU и 4 ГБ ОЗУ. Модель 4B также работает в этом приложении на таких устройствах, но производительность значительно ниже и требует больше памяти, что сложно добиться, например, в LM Studio без существенной потери качества.
