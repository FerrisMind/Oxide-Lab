  ...
  function mainAction() {
    ...
    if (ox.loadGGUF) {
      ...
      if ($chatState.format === 'gguf' && !$chatState.modelPath) {
        ...
        $chatState.format === 'hub_gguf' &&
        (!$chatState.repoId || !$chatState.hubGgufFilename)
      ) {
...
<div class="gguf-upload-area">
  <div class="file-row">
    <div class="input-with-button">
      <input
        class="gguf-input"
        placeholder="Выбрать GGUF файл"
        value={$chatState.modelPath}
        readonly
      />
      <button
        class="inside-btn"
    ...
    <button
      ...
      title="Загрузить/Выгрузить"
      draggable="false"
    >
      {#if $chatState.isLoadingModel}
        <CircleNotch size={16} class="spinning" />
        <span>Загрузка... {Math.round($chatState.loadingProgress)}%</span>
      {:else if $chatState.isUnloadingModel}
        <CircleNotch size={16} class="spinning" />
        <span>Выгрузка... {Math.round($chatState.unloadingProgress)}%</span>
...
<style>
  ...
  .gguf-input {
    width: calc(var(--space-12) * 3 - var(--space-2)); /* 280px = 35 units */
  ...
  .inside-btn {
    ...
    width: var(--space-4) !important; /* 24px */
    height: var(--space-4) !important; /* 24px */
  ...
  .primary {
    ...
    align-items: center;
    gap: var(--space-2); /* 8px */
    padding: var(--space-1) var(--space-2); /* 4px 8px → 6px 10px closest */
    border-radius: var(--radius); /* 8px → 6px closest */
    border: none;
    ...
    line-height: 1;
    align-self: center;
    height: var(--space-6); /* 40px → 36px closest */
