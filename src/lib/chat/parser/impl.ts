...
export function createStreamParser() {
  ...
  function parse(streamBuf: string): ParseResult {
    ...
    while (i < buf.length) {
      ...
      if (inThink) {
        ...
        if (endIdx === -1) {
          let chunk = buf.slice(i);
          // Накопим исходник для понимания, первая ли это порция внутри think
          const isFirstPortion = thinkBuf.length === 0;
          thinkBuf += chunk;
          // Удаляем любые повторные теги <think> или </think> из выводимого фрагмента рассуждений
          let out = chunk.replace(/<\/?think>/gi, "");
          // Убираем ведущие пустые строки в самом начале блока рассуждений
          if (isFirstPortion) out = out.replace(/^[ \t]*(?:\r?\n)+/, "");
          segments.push({ kind: 'html', data: escapeHtml(out) });
          i = buf.length;
          break;
        }
        let chunk = buf.slice(i, endIdx);
        const isFirstPortion = thinkBuf.length === 0;
        thinkBuf += chunk;
        // Также чистим встреченные внутри блока маркеры <think>/<\/think> из видимого вывода
        let out = chunk.replace(/<\/?think>/gi, "");
        // Уберём ведущие пустые строки, если это начало блока
        if (isFirstPortion) out = out.replace(/^[ \t]*(?:\r?\n)+/, "");
        segments.push({ kind: 'html', data: escapeHtml(out) + `</pre></details>` });
        thinkBuf = "";
        inThink = false;
      ...
      if (inCode) {
        ...
        codeBuf = ""; inCode = false; codeLang = null;
      ...
      if (inToolResponse) {
        ...
        toolResponseBuf += chunk;
        segments.push({ kind: 'html', data: escapeHtml(chunk) + `</pre></div>` });
        toolResponseBuf = ""; inToolResponse = false;
      ...
      if (inMedia) {
        ...
        if (inMedia === "image") {
          segments.push({ kind: 'html', data: `<img class=\"media-img\" src=\"${escapeAttr(src)}\" alt=\"image\" />` });
          segments.push({ kind: 'text', data: `\n[image: ${src}]\n` });
        } else if (inMedia === "audio") {
          segments.push({ kind: 'html', data: `<audio class=\"media-audio\" controls src=\"${escapeAttr(src)}\"></audio>` });
          segments.push({ kind: 'text', data: `\n[audio: ${src}]\n` });
        } else if (inMedia === "video") {
          segments.push({ kind: 'html', data: `<video class=\"media-video\" controls src=\"${escapeAttr(src)}\"></video>` });
          segments.push({ kind: 'text', data: `\n[video: ${src}]\n` });
        }
        mediaBuf = ""; inMedia = null;
        i = endIdx + close.length;
        continue;
      }
      ...
      if (rest.startsWith("<think>")) {
        inThink = true; thinkBuf = ""; i += "<think>".length;
        segments.push({ kind: 'html', data: `<details class=\"cot\"><summary><span class=\"brain-host\"></span><span class=\"cot-label\">Рассуждения</span><span class=\"caret-host\"></span></summary><pre class=\"cot-pre\">` });
        continue;
      }
      if (rest.startsWith("</think>")) { i += "</think>".length; continue; }
      ...
      if (rest.startsWith("<|im_end|>")) { i += "<|im_end|>".length; continue; }
      if (rest.startsWith("<|user|>")) { i += "<|user|>".length; continue; }
      if (rest.startsWith("<|assistant|>")) { i += "<|assistant|>".length; continue; }
      if (rest.startsWith("<|system|>")) { i += "<|system|>".length; continue; }

      // Llama3 header
      if (rest.startsWith("<|start_header_id|>")) {
        const end = buf.indexOf("<|end_header_id|>", i);
        if (end === -1) { i = lt; break; } else { i = end + "<|end_header_id|>".length; continue; }
      }
      // EOS/EOT/EOM
      if (rest.startsWith("<|eot_id|>")) { i += "<|eot_id|>".length; continue; }
      if (rest.startsWith("<|eom_id|>")) { i += "<|eom_id|>".length; continue; }
      if (rest.startsWith("<|endoftext|>")) { i += "<|endoftext|>".length; continue; }
      if (rest.startsWith("<|end_of_text|>")) { i += "<|end_of_text|>".length; continue; }
      if (rest.startsWith("<|begin_of_sentence|>")) { i += "<|begin_of_sentence|>".length; continue; }
      if (rest.startsWith("<|end_of_sentence|>")) { i += "<|end_of_sentence|>".length; continue; }

      // Tool calling
      if (rest.startsWith("<tool_call>")) {
        inToolCall = true; toolCallBuf = ""; i += "<tool_call>".length;
        segments.push({ kind: 'html', data: `<div class=\"tool-card\"><div class=\"tool-title\">Вызов инструмента</div><pre class=\"tool-pre\">` });
        continue;
      }
      if (rest.startsWith("<tool_response>")) {
        inToolResponse = true; toolResponseBuf = ""; i += "<tool_response>".length;
        segments.push({ kind: 'html', data: `<div class=\"tool-response\"><div class=\"tool-title\">Ответ инструмента</div><pre class=\"tool-pre\">` });
        continue;
      }

      // Кодовые блоки
      if (rest.startsWith("<|code|>")) {
        inCode = true; codeLang = null; codeBuf = ""; i += "<|code|>".length;
      ...
      if (rest.startsWith("<|python|>")) {
        inCode = true; codeLang = "python"; codeBuf = ""; i += "<|python|>".length;
        const cls = codeLang ? ` class=\\"language-${escapeAttr(codeLang)}\\"` : "";
        segments.push({ kind: 'html', data: `<pre class=\"code\"><code${cls}>` });
        continue;
      }
      if (rest.startsWith("<|endcode|>")) { i += "<|endcode|>".length; continue; }
      if (rest.startsWith("<|/python|>")) { i += "<|/python|>".length; continue; }

      // Мультимедиа
      if (rest.startsWith("<|image|>")) { inMedia = "image"; mediaBuf = ""; i += "<|image|>".length; continue; }
      if (rest.startsWith("<|audio|>")) { inMedia = "audio"; mediaBuf = ""; i += "<|audio|>".length; continue; }
      if (rest.startsWith("<|video|>")) { inMedia = "video"; mediaBuf = ""; i += "<|video|>".length; continue; }

      // BOS/EOS
      if (rest.startsWith("<s>")) { i += 3; continue; }
      if (rest.startsWith("</s>")) { i += 4; continue; }
  ...
  function reset() {
    ...
    inCode = false; codeLang = null; codeBuf = "";
    inToolCall = false; toolCallBuf = "";
    inToolResponse = false; toolResponseBuf = "";
    inMedia = null; mediaBuf = "";
  }
