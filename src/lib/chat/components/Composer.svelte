...
<div
	class={cn(
		"fixed left-1/2 w-[640px] max-w-[640px] -translate-x-1/2 px-0 transition-[top,bottom,transform] duration-300",
		hasMessages ? "bottom-20" : "top-1/2 -translate-y-1/2",
		!isLoaded && "hidden",
	)}
>
	<div class="rounded-2xl border bg-card/80 shadow-xl backdrop-blur supports-[backdrop-filter]:bg-card/70">
		<div class="flex flex-col gap-3 p-3 sm:p-4">
			...
			<div class="flex flex-col gap-3">
				<Textarea
					...
					class="composer-input min-h-[28px] w-[628px] mx-auto block self-center resize-none bg-card/80 px-0 pb-1 pt-0 mt-3 text-sm outline-none"
					style={`height:${textareaHeight}px; margin-top:4px; margin-bottom:-8px; background-color: hsl(var(--card) / 0.8);`}
				...
				<div class="flex flex-wrap items-start justify-between gap-2 px-3 -mt-2">
					<div class="flex min-w-0 flex-1 items-center gap-1.5 translate-x-1.5 [&>*]:-translate-y-1.5">
						<Button
							variant="ghost"
							size="icon-sm"
						...
						<Button
							...
							class={cn(
								...
								isLoaderPanelVisible && "text-primary",
							)}
					...
					<div class="flex flex-shrink-0 items-center gap-1.5 -translate-x-1.5 [&>*]:-translate-y-1.5">
						<Button
							variant="ghost"
							size="icon-sm"
							class={cn(
